...
;                 ]
                   ...
                   [(set-empty? locs-val)
                    ...
                    (cond 
                      [(equal? sym-meta-info #f) 
                       ...
                       (void)]
                     [else
                       (displayln (format "what's bdining in symbol table: ~a ~a" binding sym-meta-info))
                       (update-smt-symbol-table! binding sym-meta-info)])]
           ...
           (list  (make-state e benv* store* ostore measure*  kaddr time govenor-base))]
          ...
          [(forK? kont)
           ...
           (cond 
             [(empty? remaining-seq) 
              ...
              (define measure* (measure-update measure binding))
              (list (make-state exp benv* store* ostore measure* last-kaddr time govenor-base))]
             [else
              (define binding ((alloc-bind time) v-k))
              (define benv*  (benv-extend be v-k binding))
              (define store* (store-update store binding val))
              ...
              (list (make-state exp benv* store** ostore measure**  next-fork-addr time govenor-base))])]
          ['$halt empty]
     ...
     (match clo
       [(struct closure (lam benv*))
        (match lam
         ...
         (make-state exp benv** store* ostore  measure* kont-addr time govenor-base)])]))
  ...
  (define (next st)
    ...
    (match exp
      ...
      ;;;; added to support theorem provers
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
       ;; ae is supposed to be some predicate that is to put into hte govenor-base
         ...
         [( = (set-count ae-cond-set) 1)
          ...
          (cond 
            ...
            [(equal? condition #t)
              ...
              (list  (make-state tce benv store ostore measure  kont-addr time govenor-base)
                    )]
              ;; for other boolean operation result  bound to ae
            [(equal? condition #f) 
            ; (define new-govenor-base-f (set-add govenor-base neg-predicate))
              (list (make-state fce benv store ostore measure kont-addr time govenor-base))]
            ;; for binary compare operations
            [(equal? condition 'predicate)
              (define pred (addr-in-smt-sym-table? (benv-lookup benv ae)))
               (define neg-predicate `(not ,pred))
              (displayln "the condition is predicate~!!!")
               (define new-govenor-base-t (set-add govenor-base pred)) 
               (define new-govenor-base-f (set-add govenor-base neg-predicate))
                (list 
                 (make-state tce benv store ostore measure  kont-addr time new-govenor-base-t)
                 (make-state fce benv store ostore measure kont-addr time new-govenor-base-f))]
             
            )]
         [else
          (list 
           (make-state tce benv store ostore measure  kont-addr time govenor-base)
           (make-state fce benv store ostore measure  kont-addr time govenor-base))])]
       
;       (displayln (format "if's condition is ~a ~a" ae-cond-set (set-count ae-cond-set)))
;       (define pred-res (predicate? ae-cond-set))
;       (define pred (addr-in-smt-sym-table? (benv-lookup benv ae)))
;      
;       (cond
;         [(not (equal? pred #f))
;          (displayln (format "in if!!! -- got the predicate from if by govenor: ~a" pred))
;          (define neg-predicate `(not ,pred))
;          
;          (define new-govenor-base-t (set-add govenor-base pred)) 
;          (define new-govenor-base-f (set-add govenor-base neg-predicate))
;          
;          (displayln (format "in if!!! -- the govenor after true added : ~a" new-govenor-base-t))
;          (displayln (format "in if!!! -- the govenor after negation pred added : ~a" new-govenor-base-f))
;          
;          (list 
;           (make-state tce benv store ostore measure  kont-addr time new-govenor-base-t)
;           (make-state fce benv store ostore measure kont-addr time new-govenor-base-f))]
;         [else 
;          (displayln (format "in if!!! -- NOT FOUND predicate ~a from if by govenor " pred))
;          (list 
;           (make-state tce benv store ostore measure  kont-addr time govenor-base)
;           (make-state fce benv store ostore measure  kont-addr time govenor-base))])]
...
;                (local ([define t-st (make-state tce benv store ostore measure  kont-addr time govenor-base)]
;                        [define f-st (make-state fce benv store ostore measure kont-addr time govenor-base)])
;                  (cond
...
;              (set->list ((atom-eval benv store) ae)))] 
      ...
      [`(let ((,v ,ce)) ,e)
       ...
       (list (make-state ce benv new-store ostore measure*  new-kont-addr time* govenor-base))]
      ...
      [`((anf ,biop) ,ae1 ,ae2)
       ...
       (displayln (format "biop ae1 ~a and ae2 ~a" ae1-set ae2-set))
              ...
              (cond
               ...
               [(and (not (equal? int-val1 #f)) 
                ...
                (cond 
                  ...
                  [(and (symbol? ae2)
                         (symbol? ae1))
                    (displayln (format  "symbol and symbol ~a ~a" ae2 ae1))
                   (define sym-info1 (addr-in-smt-sym-table? addr-val1))
                   ;(define smt-name1 (sym-meta-sym-name sym-info1))
                   (define sym-info2 (addr-in-smt-sym-table? addr-val2))
                   ;(define smt-name2 (sym-meta-sym-name sym-info2))
                   (pretty-write smt-symbol-table)
                    (displayln (format  "symbol meta and symbolmeta ~a ~a" sym-info1 sym-info2))
                   (cond 
                     [(and (normal-meta? sym-info1)
                           (normal-meta? sym-info2))
                      (define smt-name1 (sym-meta-sym-name sym-info1))
                ...
                [else (apply kont-addr res store ostore measure time govenor-base #f)])])]
...
;         (DEBUG " the bottom value for obj-map, probably the first time allocation? " obj-map))
      ...
      [`(get-field ,ae-b ,ae-f)
       ...
       (DEBUG "d-f: " d-f)
       (displayln (format "get-field: ae-b is ~a" ae-b))
       (displayln(format "d-b: ~a" d-b))
       
       (flatten
        (for/list ([val (in-set d-b)])
          ;; yeah, this is ugly, but for the __class__ case, whose evaluation is just a  class symbol
          (when (symbol? val) 
            (displayln (format "val is symbol ~a" val))
            
            (set! val (get-elem-from-singleton-set ((atom-eval benv store) val))))
         ...
         (displayln res))
            ...
            [(struct OLoc (e t))
             ...
             (local ([define inst-om (ostore-lookup ostore val)]
               ...
               (displayln (format "inst-om: ~a"  inst-om))
               ;(DEBUG tmp-res)
               (cond
                 [(set-empty? tmp-res)
         ...
         (displayln res))
                 ...
                 [else
                   ...
                   (define res (transform-methods-in-inst-fld-reference tmp-res val) )
                    (when (equal? ae-b '$localo19)
                      (displayln exp)
                      (displayln (format "temp result: ~a" tmp-res))
                      (displayln (format "get-field else: ~a" val))
                      (displayln res))
         ...
         [;(and (set-empty? procs)
          ...
          (cond
            ...
            [(special-function? f 'len)
             ...
             (displayln (format "in the len funtion: ~a" exp))
;              (define oloc-res (singleton-type-set? ((atom-eval benv  store) (first args)) 'oloc))
;              (displayln (format "leng's argument is ~a" oloc-res))
;              (cond
;                [(not (equal? oloc-res #f)) 
;                 (define sym-res (addr-in-smt-sym-table? oloc-res))
;                 (displayln (format " the meta of the oloc-res:  ~a" sym-res))
;                 (displayln (format "symbol table is ~a: " smt-symbol-table))
;                 (define formula-to-bind
;                   (cond
;                     [(not (equal? sym-res #f)) 
;                      (define sname (sym-meta-sym-name sym-res))
;                        (displayln (format "in the len funtion-LEN: ~a" sname))
;                    ; you return the (LEN oloc-name) to be bound?
;                      `(LEN ,sname)]
;                     [else (error "no smt meta information found in (len ..))" )]))
;                 (apply kont-addr (set  'symbol-len) store ostore measure time govenor-base formula-to-bind)
;                 ]
;                  [else (error "no abstract location found in (len ..))" )])
;              
              (define lenarg-meta (get-len-sym (first args) benv))
              (displayln (format "lenarg-meta is: ~a" lenarg-meta))
       ...
       (flatten 
        (for/list ([proc (in-set procs)])
          (match proc
            ...
            [else
             (displayln (format " no matching for ~a in app " proc))
  ...
  (define (inject-cesk exp benv0 store0 ostore0 measure0)
    (make-state exp benv0 store0 ostore0 measure0 ((alloc-kont time-zero) `$halt) time-zero (set)))
