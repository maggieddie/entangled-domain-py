...
(module utils
  ...
  (define runtime-gbl-dict  
    (make-hash
     (list 
      (cons `object (make-hash (list (cons "__base__" (list))
                                     (cons "__mro__"  (list `object)))))
      (cons `List (make-hash (list   (cons "__base__"  (list `object))
                                     (cons "__mro__"  (list `List `object)))))
      (cons `set  (make-hash (list (cons "__base__" (list `object))
                                   (cons "__mro__"  (list `tuple `object)))))
      (cons `tuple (make-hash (list (cons "__base__" (list `object))
                                    (cons "__mro__"  (list `tuple `object)))))))
    )
  
  ; used to resume the scope information in the generating mro 
  (define gbl-scope-map (make-hash))
...
(define (empty-lists? lsts)
  ...
  (define (gen-class-dict attrs-vals class-name base class-dict)
    (let* ((var-attrs (list))
      ...
      res))
  ;;;;;;;;;;;;;;;;;;;;;;;;;;; entry for runtime- dict translator --------------
  (define (gen-class-dict-only-base attrs-vals class-name base class-dict)
    (let* ((newbase (eliminate-global-tag base))
           (base-class 
            (if (empty? newbase) 
                ;`object
                (list `object)
                (if (not (has-object? newbase))
                    (begin 
                      (append newbase (list `object)))
                    (begin
                       newbase)))
            )
           (res 
            (make-hash (list (cons "__base__"  base-class)))))
      res))
  
  ;; temp helper 
  ;; for every base, if it is global scope, 
  ;; we record the scope map for later use in generated mro
  
  (define (eliminate-global-tag base)
    (map (lambda (x) 
           (match x
             [`(get-global ,name) 
              (hash-set! gbl-scope-map name `(get-global ,name))
              name]
             [name name]))
         base))
  
  ;; generate the mro list with scope information into the generated code
  (define (mro-to-gen calculated-mro-lst)
    (map (lambda (x) 
           (if (hash-has-key? gbl-scope-map x)
               (begin
                 (hash-ref gbl-scope-map x)
                 )
               (begin
                 x)))
         calculated-mro-lst))
  ...
  (define (gen-constr res-suite last2 class-name class-name-scope class-dict base localsymtbl return-dict init-args indef symtbl)
    (if (has-init? last2)
        (let* (
               ...
               (inst-dict (gensym '$localo))
               (cls-dict-trans (gen-class-dict-only-base last2 class-name base class-dict))
          ...
          ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
          (hash-set! runtime-gbl-dict class-name cls-dict-trans)
          (let* ((runtime-mro (mro-trans class-name))
                 ;(gened-mro-list `(list ,@runtime-mro))
                 (gened-mro-list `(list ,@(mro-to-gen runtime-mro)))
                 )
            
            (hash-set! (hash-ref runtime-gbl-dict class-name) "__mro__" runtime-mro)
            `((lambda () 
                (begin
                  ,(gen-set-res class-name  
                                `(lambda ,(get-init-args last2) (call/ec (lambda (return) 
                                                                           ;,final-suite
                                                                           (let ((,inst-dict  ,(make-initial-obj-dict class-name)))
                                                                             ((get-field ,inst-dict "__init__") 
                                                                              ,@init-args)
                                                                             (return ,inst-dict))
                                                                           ))) indef symtbl)
                  ;(dict-set! gbl-dict (get-global ,class-name) ,class-dict-val)
                  (dict-set! gbl-dict ,class-name-scope ,class-dict-val)
                 ; (dict-set! (dict-ref gbl-dict (get-global ,class-name)) "__mro__" ,gened-mro-list)
                  (dict-set! (dict-ref gbl-dict ,class-name-scope) "__mro__" ,gened-mro-list))
                            ; (mro (get-global ,class-name)))
                )))
          )
        (let* (
               (class-dict-val (gen-class-dict last2 class-name base class-dict))
          ...
          ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
          (hash-set! runtime-gbl-dict class-name (gen-class-dict-only-base last2 class-name base class-dict))
  ...
  ; The len function works on the list, set, set, dictionary, string
  (define (gen-len)
    (define seq (gensym 'seq))
    (define count (gensym 'cnt))
    (define container (gensym 'container))
    (define entry (gensym 'entry))
   ;; not quite efficient code generated, since the other branch can be eliminated by supporting dict
   ;; is actually class. 
    `(lambda (,seq)
       (call/ec
        (lambda (return)
          (let ((,count 0))
            (cond
              [(or (py-list? ,seq) (set? ,seq) (tuple? ,seq))
               (let ((,container (dict-ref ,seq "__containerdict__")))
                 (for-each ,entry ,container
                           (set! ,count (+ 1 ,count)))
                 (return ,count))]
              [else (for-each ,entry ,seq (set! ,count (+ ,count 1)))
                             (return ,count)]))))))
           ; ,count)))))
