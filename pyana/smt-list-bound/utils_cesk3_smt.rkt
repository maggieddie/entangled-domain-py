(module utils_cesk3
  ...
  (define debug-flag #f)
  (define passed-in-fp #f)
  (define (set-test-file! fp)
    (set! passed-in-fp fp))
  ;; State Space (comments are in Haskell, but not purely)
  
  ;; type State = (Exp, BEnv, Store, OStore, Measure, KAddr, Time)
  (struct state (context benv store ostore measure 
                                kont-addr time govenor-base
                                ) #:prefab)
  ;; type BEnv = Var -> Addr
  (define empty-benv (make-immutable-hash empty))
  
  ;; type Store = Addr -> D
  ;; type OStore = OLoc -> Obj
  (define empty-store (make-immutable-hash empty))
  (define empty-ostore (make-immutable-hash empty))
  
  ;; type measure = Addr -> Abstract Num, which is either {0, 1, >=1}
  (define empty-measure (make-immutable-hash empty))
  ...
  (struct closure (lambda benv) #:prefab)  
  (struct kontP (addr) #:prefab) 
  
  ;; Data Kont = letk (var exp benv addr) | halt
  (struct letK (var exp benv addr) #:prefab)
  (struct forK (vk vv rest-seq e benv addr) #:prefab)
  
  (struct Cls (clo oloc) #:prefab)
  
  ;; Data Addr = KAddr (Exp, Time) | BAddr (Var, Time)
  (struct KAddr (exp time) #:prefab)
  (struct BAddr (var time) #:prefab)
  ...
  (struct OLoc (exp time) #:prefab); #:prefab)
...
(define (tick current-time time)   
  (define new-lst (list* current-time time))
...
(define (get-elem-from-singleton-set  s)
  (cond 
    ...
    [(> (set-count s) 1) (DEBUG "Warning: the set count is more than one!" (set-count s))]
    [else (car (set->list s))]))
...
(define (py-list? base store ostore)
  (define ty (get-container-type base store ostore))
  (cond 
    [(string? ty) 
     (string=?  ty "instPyList")]
    [else #f]))
...
(define content (list))
;; some utils
(define (get-result-file filep)
  (define fn (last (regexp-split #rx"/" filep)))
 (string-append "sv-tests-part-res/" fn "-bound-checking-res.txt"))

(define (acc-lines!  lst-format)
  (set! content (append content lst-format)))
(define (write-final! fn)
  (display-lines-to-file content fn #:mode 'text #:exists 'replace))
  
  
  ;; special treat function calls.
  ;; currnetly only the len. (lazy to remove the definiton in the translator phrase ;)
  
  (define (special-function? f-sym specified)
    (equal? f-sym specified))
  
  ;; currently only the uninterpreted vertex permutation example
  (define (stubbed-external-func? f-sym)
    (set-member? (set 'read_int 'read_verts 'len) f-sym))
  
(define (num? val)
  (equal? val 'num))
  
(define (widen-store accumulator-σ widen-with-σ)
  (for/fold
    ([σ accumulator-σ])
    ([key (hash-keys widen-with-σ)])
    (if (hash-has-key? σ key)
        (store-join σ widen-with-σ)
        (store-update σ key (hash-ref widen-with-σ key)))))
)