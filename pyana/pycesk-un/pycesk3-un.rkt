         ...
         [( = (set-count ae-cond-set) 1)
          ...
          (cond 
            ...
            [(equal? condition #t)
              ...
              (list  (make-state tce benv store ostore measure  kont-addr time )
                    )]
              ;; for other boolean operation result  bound to ae
            [(equal? condition #f) 
            ; (define new--f (set-add  neg-predicate))
              (list (make-state fce benv store ostore measure kont-addr time ))]
            ;; for binary compare operations
            [(or (equal? condition 'predicate)
                 (equal? condition 'num))
             ...
             ; (displayln (format "the condition is predicate ae ~a: ~a!!!" ae pred))
              ...
              ; (define new--f (set-add  neg-predicate))
                (list 
                 (make-state tce benv store ostore measure  kont-addr time )
                 (make-state fce benv store ostore measure kont-addr time ))]
      ...
      [`((anf ,biop) ,ae1 ,ae2)
       ...
       (cond
         ...
         [(or (arith-ops? biop) (comp-biops? biop))
          ...
          (apply kont-addr (set 'num) store ostore  measure time)]
         ;; for container
         ...
         [;(and (set-empty? procs)
          ...
          (cond
            [(special-function? f 'read-int)
             ...
             (apply kont-addr (set 3) store ostore measure time  )
             ]
            [(special-function? f 'len)
             ...
             (displayln (format "in the len funtion: ~a" exp))
              ...
              (apply kont-addr (set  'symbol-len) store ostore measure time )
              ]
