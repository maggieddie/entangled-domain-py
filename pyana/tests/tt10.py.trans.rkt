...
(program
 ...
 (define bar2 (void))
 (define bar1 (void))
 (define changedlst (void))
 (define bar3 (void))
 (define new_elem (void))
  ...
  (lambda (a b)
    (call/ec
     (lambda (return)
       (let ((c (void)))
         (set! c (+ a b))
         (return ((get-global List) (dict (0 a) (1 b) (2 c)))))))))
 (set-global!
  bar2
     ...
     (lambda (return)
       (let ()
         (let ((b11 lst))
           (let ((i12 index))
             (cond
              ((py-list? b11) (py-list-set! b11 i12 newvalue))
              ((dict? b11) (dict-set! b11 i12 newvalue)))))
         (return lst))))))
 (set-global!
  bar3
  (lambda ()
    (call/ec
     (lambda (return)
       (let ((lst (void)))
         (set! lst ((get-global bar2) ((get-global bar1) 1 2) "newvalue" 0))
         (return lst))))))
 (set-global! changedlst ((get-global bar3)))
 (set-global!
  new_elem
  (let ((e13 (get-global changedlst)))
    (let ((i14 0))
      (cond
