(module utils-cesk3-taint
  ...
  (define-struct state (context benv store ostore measure taint-store taint-ostore
  ...
  ;; type taint-store = Addr|OLoc -> {"tainted", "tainted"}
  (define empty-taint-store (make-immutable-hasheq empty))
  ...
  (define-struct forK (vk vv rest-seq taint-v rest-taints e benv addr) #:transparent)
...
;****************************** helpers in atom-eval **********************************
  ...
  ;; evaluate to the taint-store
  (define (taint-eval benv taint-store)
    (match-lambda  
      [(? lam? lam)  ;; lambda definition is untained initilly
       (set "untainted")]
      [(? ae-list? l) (set "untainted")]
      [(? void? v)     (set "untainted")]
      [(? symbol? sym)
       (cond 
         [(equal? sym '$halt)  (set "untainted")] ;;;;;;;;;;;;;;;;;;;;
         [else (store-lookup taint-store (benv-lookup benv sym))])]
       [(? KAddr? ka)
        (store-lookup taint-store ka)]
       [(? Cls? cls) (set "untainted")] ;; should be originally untained
       [(? OLoc? loc) (store-lookup taint-store loc)]
       [(? number? num) (set "untainted")]  
       [(? string? str) (set "untainted")]
       [(? boolean? b)  (set "untainted")]
       [;;;;;;;;
        (? empty? mt) (DEBUG "empty list evaluated" mt)  (set "untainted")]))
...
;; evalutes values according to the key
 (define (new-vals k-lst v-lst benv store)
  ...
  ;; evalutes values according to the key
 (define (new-taints k-lst v-lst benv taint-store)
   (define len-k (length k-lst))
   (define len-v (length k-lst))
   (cond 
     [(not (= len-k len-v)) (error "key - vals number not matched!" )]
     [else
      (for/list ([e1 k-lst]
                 [e2 v-lst])
        (cond 
          [(equal? e1 "terms") (set "tainted")]
          [(meta? e1) (set "untainted")]
          [else  ((taint-eval  benv taint-store) e2)]))]))

; ***************************** helper for dict **********************************
  (define (usr-input? ks)
    (define res (filter ((curry equal?) "terms") ks))
    (cond
      [(empty? res) #f]
      [else #t]))
...
(define (compute-py-str str num-or-str op)
  (match op
    ['+ 
     (cond 
       [(string? num-or-str) (string-append str num-or-str)]
       [else 
    ...
    ['* 
     (cond
       [(number? num-or-str)
        ...
        (DEBUG "dupilicate string times: " num-or-str)
        (copy-str-n-time str num-or-str (list))]
       [else 'void])]))
  ...
  (cond 
    [(arith-ops? op)
     (cond 
       ...
       [(and (string? v1) (string? v2)) (compute-py-str v1 v2 op)]
       [else (DEBUG "type violation!") 'void])]
    [(comp-biops? op)
     (match-n-comp v1 v2 op)]))
  
  (define (best-effort-taint-value v1 v2 op)
    (cond
      [(or (equal? v1 "tainted") (equal? v2 "tainted")) "tainted"]
      [else "untainted"]))
...
(define (map-set2 f s1 s2 op)
  ...
  ; if the container is tainted, then so does it any reference to the elements
  ; taint-elems :: {tainted, untainted} ->  {tainted, untainted} ->  {tainted, untainted}
  (define (taint-elems computed-taints base-taint-set)
    (cond
      [(not (set-empty? computed-taints))
       (cond 
         [(set-member? base-taint-set "tainted") (set "tainted")]
         [else computed-taints])]
      [else (set)]))
...
(define (get-container-elem-lst loc ostore)
  ...
  ;; the same as above but for taint-ostore
(define (get-container-elem-lst-taint loc ostore taint-ostore)
  (define obj (ostore-lookup ostore loc))
  (define obj-t (ostore-lookup taint-ostore loc))
  (DEBUG "the obj of the forloop get-container-elem-lst-taint" obj)
  (define nested-elem-dict-loc 
    (get-elem-from-singleton-set (obj-lookup (set "__containerdict__") obj)))
  (match nested-elem-dict-loc
    [(struct OLoc (e t)) 
     (DEBUG "the taint nested-elem-dict-loc")
     (DEBUG nested-elem-dict-loc)
     (define nest-dict-obj (ostore-lookup taint-ostore nested-elem-dict-loc))
     (define-values (objm elem-setm) (values (first nest-dict-obj) (second nest-dict-obj)))
     (define container-keys (sort (hash-keys elem-setm) <))
     (DEBUG "the container keys sorted is " container-keys)
     (map (λ (x) (obj-lookup (set x) nest-dict-obj)) container-keys)]
    [else
     (DEBUG "not a location in taint thwn what" nested-elem-dict-loc)
     (define container-keys (sort (hash-keys (second obj)) <))
     (DEBUG "the container keys sorted is " container-keys)
     (map (λ (x) (obj-lookup (set x) obj-t)) container-keys)]))
...
;; basicall as above but for taint-ostore
(define (flatten-container-vals-taint set-vals ostore taint-ostore)
  (foldr (λ (val lst)
           (match val
             [(struct OLoc (e t)) 
              (append lst (get-container-elem-lst-taint val ostore taint-ostore))]
             [else (DEBUG "flatten-container-vals: not container locations" val) lst]))
         (list)
         (set->list set-vals)))
...
(define (init-measure benv0)
  ...
  ;; intialize the taint-store
(define (init-taint-store benv0)
  (foldr (λ (bdaddr res-store)
           (hash-set res-store bdaddr (set))) ; yes, I choose the empty set as the bottome
         empty-store  ; which is an hash compares keys with eq
         (hash-values benv0)))
  
  ;; initialize the taint-ostore
(define (init-taint-ostore) empty-ostore)
...
(define vulnerabilities (make-hasheq empty))
...
;********************* helpers for strong update ********************
  ...
  ;;; 
  (define (proper-update-taint ae-var val-set benv taint-store measure)
    (define addr (benv-lookup benv ae-var))
    (define cardi (store-lookup measure addr))
    (cond
      [(<= cardi 1) 
       ;; sttrong-update
       (store-strong-update taint-store addr val-set)]
      [else (store-update taint-store addr val-set)]))
